...
/**
 * NestJS-style injectable service for handling DOCX document template rendering.
 * Supports Inventario General, Mantenimiento Preventivo (Laptop) and Mantenimiento Preventivo (Desktop).
 */
export class DocumentService {
  ...
  constructor() {
    ...
    // Ensure resources directory and all templates exist
    this.ensureAllTemplates();
  }

  /**
   * Generates a DOCX inventory report based on payload metadata and equipment list.
   */
  ...
  /**
   * Generates a DOCX laptop maintenance report based on payload metadata and equipment specifications.
   */
  ...
  /**
   * Generates a DOCX desktop maintenance report based on payload metadata and CPU/Keyboard/Mouse/Monitor specs.
   */
  ...
  private renderTemplate(templatePath: string, data: any): Buffer {
    try {
      ...
      try {
        ...
        console.error(`Docxtemplater rendering error in ${path.basename(templatePath)}:`, error);
      ...
      console.error(`Document Service render exception for ${path.basename(templatePath)}:`, err);
   ...
   * on the server if they do not already exist on disk.
   */
  private ensureAllTemplates(): void {
    ...
    if (!fs.existsSync(this.templateInventarioPath)) {
      console.log("Generating default template_inventario.docx at:", this.templateInventarioPath);
    ...
    if (!fs.existsSync(this.templateLaptopPath)) {
      console.log("Generating default template_mant_laptop.docx at:", this.templateLaptopPath);
    ...
    if (!fs.existsSync(this.templateDesktopPath)) {
      console.log("Generating default template_mant_desktop.docx at:", this.templateDesktopPath);
